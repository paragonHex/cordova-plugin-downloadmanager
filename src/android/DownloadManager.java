...
public class DownloadManager extends CordovaPlugin {
    ...
    private void startDownload(String message, CallbackContext callbackContext) {
        if (message != null && message.length() > 0) {
            String messageWithoutQuery = message.substring(0, message.indexOf("?"));
            String filename = messageWithoutQuery.substring(messageWithoutQuery.lastIndexOf("/")+1, messageWithoutQuery.length());
            try {
